...
const { loadPreferredTaskGroupId, setPreferredTaskGroupId } =
  useTaskGroupPreference();
...
const thumbnailImages = ref<
  Record<string, { hash: string; url: string; localUrl: string }>
>({});
const proxyImages = ref<
  Record<string, { hash: string; url: string; localUrl: string }>
>({});
...
const selectedGroup = computed(
  () =>
    taskGroups.value.find((group) => group.id === selectedGroupId.value) ??
    null,
);
...
const selectedTaskBusy = computed(() =>
  Boolean(
    selectedTask.value &&
    ["queued", "running"].includes(selectedTask.value.status),
  ),
);
const retryTaskDisabled = computed(
  () => retryingTask.value || (selectedTaskBusy.value && !shiftPressed.value),
);
const deleteTaskDisabled = computed(
  () => deletingTask.value || (selectedTaskBusy.value && !shiftPressed.value),
);
const syncSnapshotDisabled = computed(
  () =>
    syncingSnapshot.value || (selectedTaskBusy.value && !shiftPressed.value),
);
const repairLogicDisabled = computed(
  () => repairingLogic.value || selectedTaskBusy.value,
);
const userInputVariables = computed(
  () =>
    selectedTask.value?.appSnapshot.variables.filter(
...
async function initializePage() {
  try {
    ...
    error.value =
      loadError instanceof Error ? loadError.message : "加载任务分组失败";
    showingGroupPicker.value = true;
    toast.error(error.value);
  }
}
...
function initialGroupId() {
  ...
  const queryGroup = taskGroups.value.find(
    (group) => group.id === routeGroupId,
  );
  const preferredGroupId = loadPreferredTaskGroupId();
  const preferredGroup = taskGroups.value.find(
    (group) => group.id === preferredGroupId,
  );
  return (
    queryGroup?.id ?? preferredGroup?.id ?? taskGroups.value[0]?.id ?? null
  );
}
...
async function retryNode(nodeId: string, event?: MouseEvent) {
  ...
  const nodeRun = selectedTask.value.nodeRuns.find(
    (item) => item.nodeId === nodeId,
  );
  const userForced = event?.shiftKey === true;
  const force =
    userForced || (selectedTaskBusy.value && nodeRun?.status === "completed");
  if (!force && selectedTaskBusy.value && nodeRun?.status !== "completed")
    return;
  try {
    retryingNodeId.value = nodeId;
    const updated = await appApi.retryTaskNode(
    ...
    toast.error(
      retryError instanceof Error ? retryError.message : "重试节点失败",
    );
  } finally {
...
async function retryTask(event?: MouseEvent) {
  ...
  try {
    ...
    const updated = await appApi.retryTask(
      selectedTask.value.id,
      undefined,
      force,
    );
...
async function submitEditedInputs(action: "save" | "retry" = "retry") {
  ...
  try {
    ...
    if (action === "save") {
      ...
      const updated = await appApi.updateTaskInputs(
        selectedTask.value.id,
        inputs,
      );
      upsertTask(updated, { select: true });
...
async function syncSelectedTaskSnapshot(event?: MouseEvent) {
  ...
  try {
    ...
    toast.error(
      syncError instanceof Error ? syncError.message : "同步任务快照失败",
    );
  } finally {
...
async function repairSelectedTaskLogic() {
  if (!selectedTask.value || repairingLogic.value || selectedTaskBusy.value)
    return;
  try {
    ...
    toast.error(
      repairError instanceof Error ? repairError.message : "修复任务逻辑失败",
    );
  } finally {
...
async function resumeNode(nodeId: string) {
  ...
  try {
    ...
    const updated = await appApi.resumeAppTask(
      selectedTask.value.appId,
      selectedTask.value.id,
    );
    upsertTask(updated, { select: true });
    toast.success("任务已继续");
    ...
    toast.error(
      resumeError instanceof Error ? resumeError.message : "继续任务失败",
    );
  } finally {
...
function upsertTask(task: AppTaskRecord, options: { select?: boolean } = {}) {
  if (selectedGroupId.value && task.taskGroupId !== selectedGroupId.value)
    return;
...
function taskThumbnail(task: AppTaskRecord) {
  ...
  const imageInput = task.appSnapshot.variables.find(
    (variable) => variable.source === "user_input" && variable.type === "IMAGE",
  );
  if (imageInput)
    return imageUrl(
      task.variables[imageInput.key] ?? task.inputs[imageInput.key],
    );
...
function findTaskImage(
  task: AppTaskRecord,
  predicate: (value: unknown) => boolean = isImageValue,
) {
  const imageVariables = task.appSnapshot.variables.filter(
    (variable) => variable.type === "IMAGE",
  );
  for (const variable of imageVariables) {
    const value =
      task.outputs[variable.key] ??
      task.variables[variable.key] ??
      task.inputs[variable.key];
...
function validateEditedInputs() {
  for (const variable of userInputVariables.value) {
    ...
    if (variable.type === "IMAGE") {
      if (
        !editFileValues.value[variable.key] &&
        !editPreviousImageValues.value[variable.key] &&
    ...
    if (isEmptyValue(editTextValues.value[variable.key]))
      return `应用输入 $${variable.key} 不能为空`;
  }
  return "";
}
  ...
  for (const variable of userInputVariables.value) {
    if (variable.type === "IMAGE") {
      ...
      inputs[variable.key] = file
        ...
        : (previousImage ?? variable.default);
      continue;
    }

    inputs[variable.key] = parseInputValue(
      editTextValues.value[variable.key] ?? "",
      variable.type,
    );
  }
  return inputs;
...
function taskButtonClass(task: AppTaskRecord) {
  if (selectedTaskId.value === task.id)
    return "outline-0 ring-2 ring-slate-950";
  if (task.requiresManualAction)
    return "outline-[2.5px] outline-[#7bed9f] hover:outline-yellow-500";
...
async function moveTaskToGroupAction(targetGroupId: number) {
  ...
  try {
    ...
    selectedTaskId.value = tasks.value[0]?.id ?? null;
    await syncTaskRoute();
    const targetGroup = taskGroups.value.find(
      (group) => group.id === targetGroupId,
    );
    toast.success(`任务已移动到 ${targetGroup?.name ?? "目标分组"}`);
  } catch (moveError) {
    toast.error(
      moveError instanceof Error ? moveError.message : "移动任务失败",
    );
  } finally {
    ...
    <div class="flex h-full">
      <div class="relative min-w-0 flex-1">
        ...
        <div class="absolute left-4 top-4 z-30 flex items-center gap-2">
          ...
          <div
            v-if="showingGroupPicker"
            class="flex h-9 items-center rounded-md border bg-white px-3 text-sm font-medium shadow-sm"
          >
          ...
          <div
            ...
            class="flex h-9 items-center gap-1.5 rounded-md border bg-white px-3 text-sm shadow-sm"
          >
            <DropdownMenu>
              <DropdownMenuTrigger as-child>
                <button
                  type="button"
                  class="flex items-center gap-1 font-medium transition hover:text-slate-600 outline-none"
                >
            ...
            <span
              class="ml-1 rounded-full px-2 py-0.5 text-[10px] bg-slate-100 text-slate-600"
        ...
        <div class="absolute right-4 top-4 z-30 flex items-center gap-2">
          <template v-if="selectedTask && !showingGroupPicker">
            ...
            >
              <RotateCw
                class="size-4"
                :class="retryingTask ? 'animate-spin' : ''"
              />
              重试
            </Button>
            <Button
            ...
            >
              <Wrench
                class="size-4"
                :class="repairingLogic ? 'animate-pulse' : ''"
              />
              修复逻辑
            </Button>
            <Button
            ...
            <DropdownMenu v-if="taskGroups.length > 1">
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem
                  :disabled="syncSnapshotDisabled"
                  @click="syncSelectedTaskSnapshot"
                >
                  <RefreshCw
                    class="mr-2 size-4"
                    :class="syncingSnapshot ? 'animate-spin' : ''"
                  />
                  同步当前应用快照
                </DropdownMenuItem>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  v-for="group in taskGroups.filter(
        ...
        <!-- 左侧悬浮任务列表 -->
        <div
          v-if="!showingGroupPicker && tasks.length > 0"
          class="absolute left-3 px-1 top-16 z-30 flex flex-col gap-2 max-h-[calc(100vh-5rem)] overflow-y-auto overscroll-contain pb-4 scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
        >
          <button
            v-for="task in tasks"
            :key="task.id"
          ...
          >
            <img
              v-if="taskThumbnail(task)"
              :src="taskThumbnail(task)"
              :alt="`任务 #${task.id}`"
            ...
            <div
              ...
              :class="taskFallbackClass(task)"
            >
              <Image class="size-5 text-slate-600" />
            </div>
            <span
              class="absolute bottom-0 left-0 right-0 bg-white/85 text-[10px] font-medium text-slate-700"
              >#{{ task.id }}</span
            >
          </button>
        </div>
        ...
        <div class="h-full w-full">
          <div
            v-if="showingGroupPicker"
            class="h-full overflow-auto pt-20 px-6 bg-slate-50"
          >
            <div class="mb-5">
              <h1 class="text-xl font-semibold">选择任务分组</h1>
              <p class="mt-1 text-sm text-slate-500">
                会自动进入上次选择的分组，也可以从这里切换到其他分组。
              </p>
            </div>
            ...
            <div
              v-else
              class="rounded-lg border border-dashed p-8 text-center text-sm text-slate-500 bg-white"
            >
              {{ error || "暂无任务分组" }}
            </div>
          </div>
          ...
          <div
            v-else
            class="flex h-full items-center justify-center p-6 text-center text-sm text-slate-500 bg-slate-50"
          >
            {{ error || "暂无任务" }}
          </div>
        </div>
    ...
    <Dialog v-model:open="editingInputs">
      <DialogContent class="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription
            >可仅保存任务输入参数，也可保存后重置当前任务并再次执行。</DialogDescription
          >
        </DialogHeader>

        <form
          class="max-h-[70vh] space-y-4 overflow-y-auto py-2"
          @submit.prevent="submitEditedInputs('retry')"
        >
          <div
            v-if="userInputVariables.length === 0"
          ...
          <div
            v-for="variable in userInputVariables"
            :key="variable.key"
            class="space-y-2 rounded-lg border p-3"
          >
            <Label
              :for="`task-input-${variable.key}`"
            ...
            <div v-else-if="variable.type === 'IMAGE'" class="space-y-2">
              <div
                v-if="imageUrl(editPreviousImageValues[variable.key])"
                class="flex items-center gap-3 rounded-lg bg-slate-50 p-3"
              >
                <img
                  :src="imageUrl(editPreviousImageValues[variable.key])"
                  :alt="variable.name"
                ...
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  @click="clearEditImage(variable.key)"
                >
                  <Trash2 class="size-4" />
                </Button>
              </div>
              <Input
                ...
                @change="setEditFile(variable, $event)"
              />
              <p
                v-if="editFileValues[variable.key]"
            ...
            <Textarea
              ...
              class="min-h-24"
              :placeholder="
                variable.type === 'LORA_LIST' ? '输入 JSON 数组' : '输入参数值'
              "
            />
          </div>
          ...
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              :disabled="retryingTask || savingInputs"
              @click="editingInputs = false"
              >取消</Button
            >
            <Button
              type="button"
              variant="outline"
              :disabled="retryingTask || savingInputs || selectedTaskBusy"
              @click="submitEditedInputs('save')"
            >
              <Pencil
                class="size-4"
                :class="savingInputs ? 'animate-pulse' : ''"
              />
              仅保存参数
            </Button>
            <Button
